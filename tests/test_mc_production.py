...
def test_create_mc_stage_classes(get_whizard_mc_stage_config):
    """ 
    Check that when whizard setup, there are two MC production stage tasks
    """
...
def test_create_mc_stage_classes(get_madgraph_mc_stage_config):
    """ 
    Check that when madgraph setup, there are two MC production stage tasks
    """
...
def test_collect_cmd_inputs_for_magraph(get_full_madgraph_setup):
    ...
    for (raw_task, prod_cmd) in zip(run_tasks.values(), [stage1_prod_cmd, stage2_prod_cmd]):
        # Initilaise the task with the prodtype and datatype
        task = raw_task(
            prodtype=get_mc_production_types()['madgraph'], 
            datatype=datatype
        )
...
def test_collect_cmd_inputs_for_whizard(get_full_whizard_setup):
    ...
    for (raw_task, prod_cmd) in zip(run_tasks.values(), [stage1_prod_cmd, stage2_prod_cmd]):
        ...
        if task.stage == 'stage2':
            ...
            prod_cmd = prod_cmd( str(task.input_file_path), str(task.tmp_output_parent_dir / task.output_file_name) ) 
